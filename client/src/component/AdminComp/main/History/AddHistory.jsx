...
const AddHistory = () => {
  ...
  const saveNewHistory = async (e) => {
    e.preventDefault();
    const formData = new FormData();
    formData.append("file", file);
    formData.append("desc", desc);
    try {
      await axios.post("http://localhost:9900/history", formData, {
        headers: {
  ...
  return (
    <div className="h-screen bg-slate-200 p-12">
      <div className="p-5 font-poppins rounded-lg bg-white shadow-xl">
        <h1 className="font-bold text-2xl text-center">ADD NEW HISTORY</h1>
        <form onSubmit={saveNewHistory}>
          <div className="mt-2">
